...
class VisualServoNode(Node):
    ...
    def color_callback(self, msg):
        ...
        # ── [상태 2] SERVOING ───────────────────────────────────────────
            ...
            # [페이즈 3] Z — depth만 제어 (XY 유지 항 제거로 X축 이동 방지)
                ...
                if self.cv_depth_image is not None and 0 <= self.last_by < height and 0 <= self.last_bx < width:
                    ...
                    if distance_mm > 0:
                        depth_error = float(distance_mm) - 350.0
                ...
                if self.hover_stable_count >= 10:
                    self.hover_stable_count = 0
                    self.state = 'HOVERING'
                    self.get_logger().info("정렬 성공... HOVERING 진입")
        ...
        # ── [상태 3] HOVERING ───────────────────────────────────────────
            ...
            # cmd_msg.twist.linear.z = -0.05  # blind drop (하강 파지)
            cv2.putText(cv_image, "Mode: HOVERING", (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 3)
