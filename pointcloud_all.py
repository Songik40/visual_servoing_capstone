...
class PointCloudRansacTest(Node):
    def __init__(self, config):
        ...
        self.search_pan_busy = False
        self.search_pan_dir = 1.0
        self.search_pan_step = 0.10  # rad, ~5.7도
        self.search_pan_count = 0
        ...
        self.tracking_phase = 'X'   # 'X' → 'Y' 순차 보정
        self.pixel_tolerance = 20
        self.y_pixel_tolerance = 40         # Y는 X보다 넓게 허용
        self.y_correction_timeout = 2.0     # 최대 2초만 Y 보정
        self.y_correction_start_time = None
    ...
    def move_up_down_by_bbox_error(self, error_y):
        if self.current_joint_state is None:
            self.get_logger().warn("Y 보정 실패: joint_state 없음")
            return
        ...
        # error_y > 0: bbox가 화면 아래 → shoulder_lift 증가(팔 내림)
        # error_y < 0: bbox가 화면 위  → shoulder_lift 감소(팔 올림)
        step = 0.004  # rad, ~0.23도 (shoulder_lift는 민감하므로 작게)
        ...
        self.get_logger().warn(
            f"SCALED Y 보정: error_y={error_y}, shoulder_lift step={np.degrees(step):.2f} deg"
        )
    ...
    def search_z_motion(self):
        ...
        if self.advancing or self.is_planning or self.goal_active:
            return
        if self.search_z_busy or self.search_pan_busy:
            return
    ...
    def search_pan_motion(self):
        if time.time() - self.node_start_time < self.z_search_start_delay:
            return

        if self.advancing or self.is_planning or self.goal_active:
            return
        if self.search_pan_busy or self.search_z_busy:
            return

        now = time.time()
        if hasattr(self, "last_search_pan_time"):
            if now - self.last_search_pan_time < 3.0:
                return

        if self.current_joint_state is None:
            self.get_logger().warn("PAN SEARCH 실패: joint_state 없음")
            return

        self.search_pan_busy = True
        self.last_search_pan_time = now
        ...
        target_pos = [float(pos_dict[name]) for name in joint_order]
        target_pos[0] += self.search_pan_step * self.search_pan_dir
        ...
        self.get_logger().warn(
            f"PAN SEARCH: dir={self.search_pan_dir:+.0f}, "
            f"step={np.degrees(self.search_pan_step):.1f}deg, "
            f"count={self.search_pan_count}"
        )

        self.search_pan_timer = self.create_timer(
            2.0 + self.search_z_wait_sec,
            self.finish_search_pan_once
        )

    def finish_search_pan_once(self):
        if hasattr(self, "search_pan_timer") and self.search_pan_timer is not None:
            self.search_pan_timer.cancel()
            self.search_pan_timer = None

        self.search_pan_count += 1
        if self.search_pan_count % 2 == 0:
            self.search_pan_dir *= -1.0

        self.search_pan_busy = False
        self.get_logger().warn("PAN SEARCH 완료 → 다음 방향 준비")
    ...
    def color_callback(self, msg):
        ...
        if self.tracking_mode and not self.tracking_done:
            ...
            if self.tracking_phase == 'X':
                cv2.line(img, (image_center_x, h // 2), (cx_color, cy_color), (0, 255, 255), 2)
                cv2.putText(img, f"X TRACK error={error_x}", (40, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                self.get_logger().warn(
                    f"YOLO X TRACK: bbox_cx={cx_color}, img_cx={image_center_x}, error_x={error_x}"
                )

                if abs(error_x) < self.pixel_tolerance:
                    self.tracking_phase = 'Y'
                    self.get_logger().warn("X 보정 완료 → Y 보정 시작")
                else:
                    self.move_side_by_bbox_error(error_x)

            elif self.tracking_phase == 'Y':
                # Y 보정 시작 시각 기록
                if self.y_correction_start_time is None:
                    self.y_correction_start_time = time.time()

                y_elapsed = time.time() - self.y_correction_start_time
                y_done = (abs(error_y) < self.y_pixel_tolerance
                          or y_elapsed >= self.y_correction_timeout)

                cv2.line(img, (w // 2, image_center_y), (cx_color, cy_color), (255, 255, 0), 2)
                cv2.putText(img, f"Y TRACK error={error_y} t={y_elapsed:.1f}s", (40, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
                self.get_logger().warn(
                    f"YOLO Y TRACK: bbox_cy={cy_color}, img_cy={image_center_y}, "
                    f"error_y={error_y}, elapsed={y_elapsed:.1f}s"
                )
                ...
                if y_done:
                    ...
                    reason = "허용범위" if abs(error_y) < self.y_pixel_tolerance else "타임아웃"
                    self.get_logger().warn(f"Y 보정 완료({reason}) → 15cm 전진")
                    self.send_forward_to_bottle_15cm()
                    self.show_debug_image(img)
                    return
                else:
                    self.move_up_down_by_bbox_error(error_y)
